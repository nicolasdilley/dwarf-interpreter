defmodule Dwarf.Env do
	...
	def add(env,{:dec,line,[type,name,val]}) do
		...
		if !updated do
			...
			raise "Dynamic error: var #{name} was already declared can not declare twice the same variable. on line #{line}"
		end
	end
	...
	def update(env,{:assign,line,[varName,val]}) do
		{updated,new_env} = List.foldl(env,{false,[]},fn var, {updated,newEnv} -> 
			case var do
	...
	def get(env,varName) do
	  ...
	  case var do
	  	nil -> {false,{}}
		{_,{_,_,[_name,value]}} -> {true,value} # returning the value of a var
	  	{_,a} -> {true,a} # returning the function
	  end
	end
